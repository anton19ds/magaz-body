...
<section id="history_orders">
    <div class="container">
            ...
            <div class="infoproducts__main">
                ...
                <div class="table_history_orders">
                    <div class="table_orders__line">
                        <p class="orders_line__number_order"><?= Yii::t('app', 'order') ?> №</p>
                        <p class="orders_line__date_order"><?= Yii::t('app', 'date') ?></p>
                        <p class="orders_line__status_order"><?= Yii::t('app', 'statys') ?></p>
                        <p class="orders_line__price_order"><?= Yii::t('app', 'total') ?></p>
                        <p class="orders_line__actions"><?= Yii::t('app', 'action') ?></p>
                    </div>


                    <?php $chet = 1; ?>
                    <?php foreach ($userOrders as $key => $element): ?>
                        <div class='order_payed <?= (OrderStatus::STATUS_FAILED == $element->orderStatus->status ? "disabled": "")?>'>
                            <a href="https://anticandida.com/<?= $lang?>/user/order" class="table_orders__line">
                                ...
                                <p class="orders_line__status_order">
                                    <?php if (OrderStatus::STATUS_NEW == $element->orderStatus->status): ?>
                                        <?= Yii::t('app', 'status-new') ?>
                                    <?php endif; ?>
                                    <?php if (OrderStatus::STATUS_PAY == $element->orderStatus->status): ?>
                                        <?= Yii::t('app', 'status-pay') ?>
                                    <?php endif; ?>
                                    <?php if (OrderStatus::STATUS_CLOSE == $element->orderStatus->status): ?>
                                        <?= Yii::t('app', 'status-end') ?>
                                    <?php endif; ?>
                                    <?php if (OrderStatus::STATUS_RETURN == $element->orderStatus->status): ?>
                                        <?= Yii::t('app', 'status-return') ?>
                                    <?php endif; ?>
                                    <?php if (OrderStatus::STATUS_FAILED == $element->orderStatus->status): ?>
                            ...
                            <?php if (OrderStatus::STATUS_FAILED != $element->orderStatus->status): ?>
                                ...
                                <a class="orders_line__actions_view"
                                    href="https://anticandida.com/<?= $lang ?>/user/order/<?= $element->id; ?>"><?= Yii::t('app', 'view') ?></a>
                                    <?php endif; ?>
                                
                                <?php if (OrderStatus::STATUS_NEW == $element->orderStatus->status): ?>
                                    
                                    <?php $urlPayment = '';
                                    if ($element->ordersMeta->payment_type == Cart::Inteleckt) {
                                        
                                        if ($element->paymentDataNewOreder()) {
                                            var_dump('123');
                                            $urlPayment = "https://merchant.intellectmoney.ru/?InvoiceId=" . $element->paymentDataNewOreder()->paymentId;
                                        }
                                    } else if ($element->ordersMeta->payment_type == Cart::CARD) {
                                        $urlPayment = 'https://anticandida.com/' . $lang . '/' . "payment-card-succes?orderId={$element->uuid}";
                                    }else if($element->ordersMeta->payment_type == Cart::TRISBY){
                                        //$urlPayment = "https://pay.trisbee.com/bodybalanceclinic/";
                                        $urlPayment = "https://anticandida.com/cs/shop/payment-trisby-succes?orderId={$element->uuid}";
                                    }
                                    ?>
                                    
                                    <?php if ($urlPayment): ?>
                                        
                                        <a class="orders_line__actions_pay" style="display:block" href="<?= $urlPayment; ?>" target="_blank"><?= Yii::t('app', 'pay') ?></a>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <?php else: ?>
            <div class="infoproducts__main no-orders-block">
                <h1><?= Yii::t('app', 'history-orders') ?></h1>
                <div class="blue_notification">
                    ...
                    <p class="title_blue_notification"><?= Yii::t('app', "desc-magaz-2") ?></p>
                    <div class="description_blue_notification">
                        <p><?= Yii::t('app', "desc-magaz-1") ?></p>
                    </div>
                    <a href="/<?= $lang ?>" class="blue_notification_button home-button"><?= Yii::t('app', "in-magaz") ?></a>
                </div>
            </div>
...
$(document).on("click", "a.orders_line__actions_view", function(e){
    e.preventDefault();
