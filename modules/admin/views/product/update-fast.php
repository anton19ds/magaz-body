    ...
    <td colspan="9">
        ...
        <div class="row g-0 mb-3">
            ...
            <div class="col-12 g-0">
                <div class="form-group field-product-productName">
                <strong>Отображение на версии: </strong>
                <?php $arrayData = ArrayHelper::map($model->viewProduct, 'tag', 'status');
                $lart = [];
                ?>
                
                <?php foreach($arrayData as $el => $v){
                    $lart[strtoupper($el)] = $v;
                }?>
                <?php foreach ($LangTag as $elem): ?>
                    <?php
                    $checked = false;
                    if (!empty($model->viewProduct)) {
                        if ((isset($lart[$elem['tag']]) && $lart[$elem['tag']] == 1) ||
                            (isset($lart[strtoupper($elem['tag'])]) && $lart[strtoupper($elem['tag'])] == 1) ) {
                            $checked = true;
                        }
                    }
                    ?>
                    <?= Html::checkbox("View[{$elem['tag']}]", '1', ['label' => $elem['tag'], 'checked' => $checked, 'uncheck' => '0',]) ?>
                <?php endforeach; ?>
                </div>
            </div>
